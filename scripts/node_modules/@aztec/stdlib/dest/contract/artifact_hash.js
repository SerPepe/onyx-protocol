...
export function computeArtifactMetadataHash(artifact) {
    return sha256Fr(Buffer.from(deterministicStringify({
        name: artifact.name,
...
async function computeFunctionLeaves(artifact, fnType) {
    const selectors = await Promise.all(artifact.functions.filter((f) => {
        if (fnType === FunctionType.UTILITY) {
            return f.functionType === FunctionType.UTILITY || f.functionType === 'unconstrained' || f.functionType === FunctionType.UNCONSTRAINED;
        }
        return f.functionType === fnType;
    }).map(async (f) => ({
        ...f,
...
export function computeFunctionMetadataHash(fn) {
    if (fn.return_type) {
        return sha256Fr(Buffer.from(deterministicStringify(fn.return_type), 'utf8'));
    }
